...
public class MinaServerHandle extends IoHandlerAdapter {
	...
	public void sessionOpened(IoSession session) throws Exception {
		...
		// count++;
		// Debug.log("The " + count + " client connected! address : " +
	...
	public void addClientUserToTable(IoSession ioSession) {
		// 已有就不加进来了
		if (serverModel.getClientUserFromTable(ioSession.getRemoteAddress().toString()/*+ ((InetSocketAddress) ioSession.getRemoteAddress()).getPort()*/) != null) {
			Debug.log("ServerNetwork", "User exist when Save user into Table!");
			logger.error("添加时用户已存在");
			return;
		}

		try {
			ClientUser clientUser = serverModel.getClientUserFromTable(ioSession);
			if (clientUser == null) {
				Debug.log("ServerNetwork",
						"Find new User(" + ioSession.getRemoteAddress() + ") connected,save into table");
				serverModel.addClientUserToTable(ioSession, new ClientUser(ioSession));
			} else {
				Debug.log("ServerNetwork",
						"Find  User(" + ioSession.getRemoteAddress() + ") reconnected,update  table");
				clientUser.ioSession = ioSession;
			}
